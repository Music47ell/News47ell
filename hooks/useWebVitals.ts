...
export const useWebVitals = (vitals = ['CLS', 'FCP', 'FID', 'LCP', 'TTFB']): MetricPlus[] => {
  const [metrics, setMetrics] = useState<MetricPlus[]>([])
  const unsupported = useRef<string[]>([])
  ...
  const handleReport = useCallback((metric: Metric) => {
    ...
    if (metric?.value && needsImprovement && metric?.value <= needsImprovement) {
    ...
    if (typeof window !== 'undefined') {
      vitals.forEach((vital) => {
        ...
        if (
          ...
          !PerformanceObserver.supportedEntryTypes.includes(metricConfig?.observerEntryType)
        ) {
          console.error(
            `${METRIC_CONFIG.get(vital)?.longName || vital} not supported in this browser`
          )
          if (unsupported.current.indexOf(vital) === -1) {
            unsupported.current.push(vital)
          }
          return
        }
  ...
  const sortedMetrics: MetricPlus[] = useMemo(() => {
    return vitals.map(
      (v) =>
        metrics.find((m) => m.name === (v as Metric['name'])) || {
          name: v as Metric['name'],
          loading: true,
          supported: !unsupported.current.includes(v),
          ...METRIC_CONFIG.get(v),
        }
    )
  }, [metrics, vitals])
