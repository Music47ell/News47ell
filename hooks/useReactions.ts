	...
	const toggleUserLike = useCallback(async () => {
		mutate((data) => {
			...
			return data.userLikes
				? { userLikes: false, likes: data.likes - 1 }
				: { userLikes: true, likes: data.likes + 1 }
		}, false)
	...
	const toggleUserDislike = useCallback(async () => {
		mutate((data) => {
			...
			return data.userDislikes
				? { userDislikes: false, dislikes: data.dislikes - 1 }
				: { userDislikes: true, dislikes: data.dislikes + 1 }
		}, false)
